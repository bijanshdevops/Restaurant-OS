...
| 2026-09-18 | Engineering | Delivered the product "Phase 18" feature set (cash flow & bank/cash reconciliation): a new `FinancialAccount` concept (cash till / bank / payment gateway) sits on top of Phase 7's existing single-entry `Transaction` ledger, letting a transaction be attributed to *which* account the money actually moved through — a gap surfaced by codebase research before scoping (Phase 7's accounting module and the separate Modian e-invoicing module already cover categorized P&L/VAT/tax-document submission, so this phase deliberately did not rebuild any of that). Three protected accounts («صندوق نقد» cash, «بانک» bank, «درگاه پرداخت آنلاین» gateway) are seeded with fixed IDs, mirroring Phase 7's system-category seeding; `ADMIN`/`ACCOUNTANT` can also create custom accounts, per the user's confirmed scope. `createOrder` (POS) gained a backward-compatible `paymentMethod` parameter, surfaced as a new نقد/کارت toggle on POS checkout, linking the order's income transaction to the correct account (POS previously never recorded how a sale was paid); online/QR orders always link to the gateway account, since they only ever settle through ZarinPal — both per the user's confirmed scope. A new `AccountTransfer` model (my own architectural addition, not one of the three scoping questions asked) records moves between accounts and is deliberately kept structurally separate from `Transaction` so it never distorts Phase 7's P&L/VAT figures. A new `AccountReconciliation` model records a staff-counted balance against the ledger-computed balance and its variance; per the user's confirmed scope, no adjusting transaction is ever auto-created to zero out a mismatch. Deliberately out of scope: automatic account-linking covers only POS and online/QR (purchase-order receipts and payroll runs keep posting unclassified by default); accounts are global, not per-branch. See [CHANGELOG](CHANGELOG.md) and the "Cash Flow & Bank/Cash Reconciliation (Phase 18)" section in [README.md](README.md). This is a feature-delivery milestone tracked separately from the architecture-governance phases above. |
