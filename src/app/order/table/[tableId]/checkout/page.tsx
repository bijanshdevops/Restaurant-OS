...
import { createDineInQrOrder } from '@/app/actions/order';
...
/**
 * فاز ۱۶: تسویه‌حسابِ سفارشِ QR روی میز — معادلِ CheckoutPage آنلاین
 * (/order/checkout)، با این تفاوت‌ها: بدونِ فیلدِ آدرسِ تحویل، بدونِ
 * ردیف‌های هزینه‌ی بسته‌بندی/ارسال (چون صفر است — نک. createDineInQrOrder
 * در order.ts)، و صدا زدنِ createDineInQrOrder به‌جای createOnlineOrder.
 * پرداخت هنوز دقیقاً همان initiatePayment/درگاهِ زرین‌پالِ مشترک است.
 */
export default function DineInQrCheckoutPage() {
  ...
  useEffect(() => {
    ...
    getTableForOrder(tableId).then((res) => {
      if (res.success && res.table) setTable({ number: res.table.number, branchName: res.table.branchName });
    });
  }, [tableId]);

  useEffect(() => {
    if (lines.length === 0 && !isSubmitting) {
      router.replace(`/order/table/${tableId}`);
    }
  }, [lines.length, isSubmitting, router, tableId]);
  ...
  const handlePay = async () => {
    ...
    try {
      const orderRes = await createDineInQrOrder(
        tableId,
    ...
    <div className="min-h-screen bg-gray-50 font-sans" dir="rtl">
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <span className="font-semibold text-lg text-gray-900 block leading-tight">تسویه‌حساب</span>
            {table && (
              <span className="text-xs text-gray-500 font-medium">میز {toPersianDigits(table.number)} — {table.branchName}</span>
            )}
          </div>
          <button onClick={() => router.push(`/order/table/${tableId}`)} className="text-sm text-blue-600 font-bold">
            بازگشت به منو
          </button>
        </div>
      </header>
